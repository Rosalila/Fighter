...
Transicion* Estado::getNextTransicion(irr::core::map<stringw,stringw>*strings,irr::core::map<stringw,stringw>*strings_contrario)
{
    vector<int>transiciones_validas;
    for(int i=0;i<(int)transiciones.size();i++)
    {
        if(transiciones[i]->cumpleCondiciones(strings,strings_contrario))
        {
